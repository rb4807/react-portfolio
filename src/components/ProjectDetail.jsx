...
const ProjectStats = ({ project }) => {
    const techStackCount = project.techStackCount || 0;  // Default to 0 if not provided
    const packageCount = project.packageCount || 0;      // Default to 0 if not provided
      ...
      <div className="relative">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-16">
          <div className="flex items-center space-x-2 md:space-x-4 mb-8 md:mb-12 animate-fadeIn">
            <button
              onClick={() => navigate(-1)}
              className="group inline-flex items-center space-x-1.5 md:space-x-2 px-3 md:px-5 py-2 md:py-2.5 bg-white/5 backdrop-blur-xl rounded-xl text-white/90 hover:bg-white/10 transition-all duration-300 border border-white/10 hover:border-white/20
                    ...
                    {Object.entries(project.TechStack).map(
                      ([category, technologies], index) => (
                        <div key={index} className="tech-category">
                          <strong className="text-sm md:text-base text-blue-200">
                            {category.replace(/([A-Z])/g, " $1").toUpperCase()}{" "}
                            :
                          </strong>
                          <span className="pl-1 text-sm md:text-base text-gray-400">
                            {technologies.join(", ")}
                          </span>
                        </div>
                      )
